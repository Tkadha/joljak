...
cbuffer cbCameraInfo : register(b1)
{
    ...
    matrix gmtxPlayer2TorchShadowTransform;
    matrix gmtxPlayer3TorchShadowTransform;
...
float4 Lighting(MaterialInfo material, float3 posW, float3 normalW,
                ...
                Texture2D torch1ShadowMap,
                Texture2D torch2ShadowMap,
                Texture2D torch3ShadowMap)
{
    ...
    for (int i = 0; i < gnLights; ++i)
    {
        ...
        else if (gLights[i].m_nType == POINT_LIGHT && !gIsDaytime)
        {
            if (i == 1) // 플레이어 1 횃불
            {
                float4 torchShadowPosH = mul(float4(posW, 1.0f), gmtxTorchShadowTransform);
                shadowFactor = CalcShadowFactor(torchShadowPosH, torch1ShadowMap);
            }
            else if (i == 2) // 플레이어 2 횃불
            {
                float4 torchShadowPosH = mul(float4(posW, 1.0f), gmtxPlayer2TorchShadowTransform);
                shadowFactor = CalcShadowFactor(torchShadowPosH, torch2ShadowMap);
            }
            else if (i == 3) // 플레이어 3 횃불
            {
                float4 torchShadowPosH = mul(float4(posW, 1.0f), gmtxPlayer3TorchShadowTransform);
                shadowFactor = CalcShadowFactor(torchShadowPosH, torch3ShadowMap);
            }
        }
