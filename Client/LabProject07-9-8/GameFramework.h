...
private:
	...
	// G-Buffer 텍스처 리소스 배열
	Microsoft::WRL::ComPtr<ID3D12Resource> m_ppd3dGBufferTextures[GBUFFER_COUNT];

	// 각 G-Buffer 텍스처를 가리킬 RTV(Render Target View)의 CPU 핸들 배열
	D3D12_CPU_DESCRIPTOR_HANDLE m_pd3dGbufferRtvCPUHandles[GBUFFER_COUNT];

	// 조명 패스에서 사용할 SRV(Shader Resource View)의 GPU 핸들
	D3D12_GPU_DESCRIPTOR_HANDLE m_d3dGbufferSrvGPUHandle;
};
