...
public:
	...
	// OBB Î≥Ä???âÎ†¨???ÅÏàò Î≤ÑÌçº
	ID3D12Resource* m_pd3dcbOBBTransform = nullptr;
	XMFLOAT4X4* m_pcbMappedOBBTransform = nullptr; // ÎßµÌïë???¨Ïù∏??
...
public:
	...
	XMFLOAT3 m_xmf3Gravity = XMFLOAT3(0.0f, -980.0f, 0.0f); // Ï§ëÎ†• Í∞Ä?çÎèÑ (Ï°∞Ï†ï ?ÑÏöî)
	bool m_bOnGround = false;
	float m_fLifeTime = 15.0f; // Î∞îÎã•???®Ïñ¥Ïß????¨ÎùºÏßÄÍ∏∞ÍπåÏßÄ ?úÍ∞Ñ (Ï¥?
	float m_fElapsedAfterLanding = 0.0f;

	CHeightMapTerrain* m_pTerrainRef = nullptr; // ÏßÄ??Ï∞∏Ï°∞ (Ï∂©Îèå Í∞êÏ???

	CItemObject() { m_objectType = GameObjectType::Item; };
	virtual ~CItemObject() {};

	virtual void Animate(float fTimeElapsed) override; // Î¨ºÎ¶¨ Î∞??òÎ™Ö Ï≤òÎ¶¨
...
// ------- ?∞Îü¨ÏßÄ???†ÎãàÎ©îÏù¥??-------
	...
	// ?∞Îü¨ÏßÄ???†ÎãàÎ©îÏù¥???úÏûë ?®Ïàò
	void StartFalling(const XMFLOAT3& hitDirection); // ?åÎ†à?¥Ïñ¥??Í≥µÍ≤© Î∞©Ìñ• ?±ÏùÑ Î∞õÏùÑ ???àÏùå

	// Îß??ÑÎ†à???†ÎãàÎ©îÏù¥???ÖÎç∞?¥Ìä∏
	virtual void Animate(float fTimeElapsed) override;

	bool IsFalling() const { return	m_bIsFalling; }
	bool HasFallen() const { return m_bHasFallen; }

	bool m_bIsFalling = false;       // ?ÑÏû¨ ?∞Îü¨ÏßÄ???†ÎãàÎ©îÏù¥??Ï§ëÏù∏Í∞Ä?
	bool m_bHasFallen = false;       // ?¥Î? ?∞Îü¨Ïß??ÅÌÉú?∏Í??
	float m_fFallingDuration = 2.5f;  // ?∞Îü¨ÏßÄ????Í±∏Î¶¨???úÍ∞Ñ (Ï¥?
	float m_fFallingTimer = 0.0f;     // ?∞Îü¨ÏßÄÍ∏??úÏûë????ÏßÄ???úÍ∞Ñ
	XMFLOAT3 m_xmf3FallingAxis;       // ?åÏ†Ñ Ï∂?(?∞Îü¨ÏßÄ??Î∞©Ìñ• Í≤∞Ï†ï)
	float m_fCurrentFallAngle = 0.0f; // ?ÑÏû¨ÍπåÏ? ?åÏ†Ñ??Í∞ÅÎèÑ
	float m_fTargetFallAngle = XM_PIDIV2; // Î™©Ìëú ?åÏ†Ñ Í∞ÅÎèÑ (90??

	// ?∞Îü¨ÏßÄÍ∏??úÏûë???åÏùò Ï¥àÍ∏∞ m_xmf4x4ToParent Í∞íÏùÑ ?Ä??(?ÅÎ? Î≥Ä??Í∏∞Ï?)
	XMFLOAT4X4 m_xmf4x4InitialToParent;

	// ?åÏ†Ñ??Ï§ëÏã¨??(?òÎ¨¥ Î∞ëÎèô Î∂ÄÎ∂? Î°úÏª¨ Ï¢åÌëúÍ≥?Í∏∞Ï?)
	// Î™®Îç∏???êÏ†ê???¥Î? Î∞ëÎèô?¥ÎùºÎ©?(0,0,0) ?¨Ïö© Í∞Ä??
