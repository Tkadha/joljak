...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	ThrowIfFailed(m_pd3dDevice->CreateDescriptorHeap(&d3dDescriptorHeapDesc, IID_PPV_ARGS(mRtvHeap.GetAddressOf())));
	::gnRtvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

	d3dDescriptorHeapDesc.NumDescriptors = 1;
	d3dDescriptorHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
	ThrowIfFailed(m_pd3dDevice->CreateDescriptorHeap(&d3dDescriptorHeapDesc, IID_PPV_ARGS(mDsvHeap.GetAddressOf())));
	::gnDsvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
}

void CGameFramework::CreateRenderTargetViews()
{
	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = mRtvHeap->GetCPUDescriptorHandleForHeapStart();
...
void CGameFramework::CreateDepthStencilView()
{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle = mDsvHeap->GetCPUDescriptorHandleForHeapStart();
