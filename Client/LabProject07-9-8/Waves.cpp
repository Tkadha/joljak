...
void Waves::Update(float dt)
{
    ...
    if (t >= mTimeStep)
    {
        ...
        concurrency::parallel_for(1, mNumRows - 1, [this](int i)
            //for(int i = 1; i < mNumRows - 1; ++i)
            {
                for (int j = 1; j < mNumCols - 1; ++j)
                {
                    ...
                    // Note how we can do this inplace (read/write to same buffer)
                    // because we won't need prev_ij again for the current update
                    // loop.
        ...
        //
        // Compute normals after height field has been updated.
        //
        concurrency::parallel_for(1, mNumRows - 1, [this](int i)
            //for(int i = 1; i < mNumRows-1; ++i)
            {
